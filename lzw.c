...
void lzw_len_update(lzw_stream_p s) {
  DEBUG(1, "Updating length: %d %d -> %d\n", s->next_key, s->data_length, s->data_length+1);
  s->data_length++;
...
void lzw_destroy_state(lzw_stream_p s) {
  ...
  for (uint32_t i = 0; i < s->next_key; ++i) {
    if (s->data[i].data) free(s->data[i].data);
  }
  free(s->data);
  s->next_key = 1;
  s->data_length = 1;
...
void end(lzw_stream_p s) {
  ...
  while (s->bufferSize != 0) {
    if (lzw_debug_level > 1) debug_emit(s, 0, 1);
    emit(s, 0, 1);
  }
}
...
lzw_stream_p lzw_init(lzw_reader_t r, lzw_emitter_t w) {
